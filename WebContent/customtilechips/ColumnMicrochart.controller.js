...
(function() {
	...
	sap.ui.controller("customtilechips.ColumnMicrochart", {
		...
		successHandleFn: function(oResult) {
			...
			// Prepare emp object:
			oResult.results = {};
			oResult.results.values = [];
			oResult.results.colors = [];
			oResult.results.labels = [];
			
			for (var key in oResult) {
				if (oResult.hasOwnProperty(key)) {
					if(key.includes("value")){
						oResult.results.values = oResult[key].split(";");
					}else if (key.includes("color")){
						oResult.results.colors = oResult[key].split(";");
					}else if (key.includes("labels")){
						oResult.results.labels = oResult[key].split(";");
					}
				}
			}
			
			var microChart = sap.ui.getCore().byId(this.getView().sId+"--columnMicroChart");
			microChart.destroyColumns();
			for (var i = 0; i < oResult.results.values.length; i++) {
				microChart.insertColumn(
					new sap.suite.ui.microchart.ColumnMicroChartData({
						value: parseFloat(oResult.results.values[i]),
						label: oResult.results.labels[i],
						color: oResult.results.colors[i]
					})
				);
			}
